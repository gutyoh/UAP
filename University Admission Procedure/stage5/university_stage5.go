...
func main() {
	...
	file, err := os.Open("C:\\Users\\mrgut\\Documents\\UAP\\University Admission Procedure\\stage5\\applicant_list_5.txt")
	if err != nil {
		log.Fatal(err)
	}
	defer file.Close()
	...
	for i := 0; i < len(s); i++ {
		if !isInSlice(used, s[i].name) && s[i].departments[0] == "Biotech" && count["Biotech"] < nApplicants {
			final["Biotech"] = append(final["Biotech"], s[i].name+" "+s[i].lastName+" "+strconv.FormatFloat(s[i].score[1], 'f', 2, 64))
	...
	sort.Slice(s, func(i, j int) bool {
		if s[i].score[3] != s[j].score[3] {
			return s[i].score[3] > s[j].score[3]
		}
		return s[i].name < s[j].name
	})
	...
	for i := 0; i < len(s); i++ {
		if !isInSlice(used, s[i].name) && s[i].departments[0] == "Engineering" && count["Engineering"] < nApplicants {
			final["Engineering"] = append(final["Engineering"], s[i].name+" "+s[i].lastName+" "+strconv.FormatFloat(s[i].score[3], 'f', 2, 64))
			used = append(used, s[i].name)
			count["Engineering"]++
		}
	}
	...
	sort.Slice(s, func(i, j int) bool {
		if s[i].score[0] != s[j].score[0] {
			return s[i].score[0] > s[j].score[0]
		}
		return s[i].name < s[j].name
	})
	...
	for i := 0; i < len(s); i++ {
		if !isInSlice(used, s[i].name) && s[i].departments[0] == "Physics" && count["Physics"] < nApplicants {
			final["Physics"] = append(final["Physics"], s[i].name+" "+s[i].lastName+" "+strconv.FormatFloat(s[i].score[0], 'f', 2, 64))
			used = append(used, s[i].name)
			count["Physics"]++
		}
	}
